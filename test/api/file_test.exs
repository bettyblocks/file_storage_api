defmodule FileStorageApi.FileTest do
  ...
  describe "atom based env config" do
  ...
  describe "config map based config" do
    @connection %{
      engine: Mock,
    ...
    test "able to upload a file" do
      ...
      expect(FileMock, :upload, fn "block-store-container", @connection, ^file_path, _blob_name, _ ->
        {:ok, %{}}
      end)

      assert {:ok, %{}} == File.upload("block-store-container", file_path, "testfile", connection: @connection)
    end

    test "upload a file with mime type" do
      file_path = "./test/support/test_icon.png"

      expect(FileMock, :upload, fn "block-store-container", @connection, _, filename, [content_type: "image/png"] ->
        {:ok, filename}
      end)

      assert {:ok, Path.basename(file_path)} ==
               File.upload("block-store-container", file_path, "test_icon.png", connection: @connection)
    end

    test "upload a file with mime type application/javascript" do
      file_path = "./test/support/hello.js"

      expect(FileMock, :upload, fn "block-store-container",
                                   @connection,
                                   _,
                                   filename,
      ...
      assert {:ok, Path.basename(file_path)} ==
               File.upload("block-store-container", file_path, "hello.js", connection: @connection)
    end
    ...
    test "not creating a container if not forcing on a failed upload" do
      ...
      assert {:file_upload_error, "Some Error"} ==
               File.upload("block-store-container", file_path, "testfile",
    ...
    test "creating a container once, if file upload fails because of it" do
      ...
      FileMock
      |> expect(:upload, 1, fn "block-store-container", @connection, ^file_path, _blob_name, _ ->
        {:error, :container_not_found}
      end)
      |> expect(:upload, 1, fn "block-store-container", @connection, ^file_path, _blob_name, _ ->
        {:ok, "file uploaded!"}
      ...
      assert {:ok, "file uploaded!"} ==
               File.upload("block-store-container", file_path, "testfile", connection: @connection)
    end
    ...
    test "returning the error if creating container didn't help" do
      ...
      assert {:file_upload_error, :container_not_found} ==
               File.upload("block-store-container", file_path, "testfile", connection: @connection)
    end
    ...
    test "able to request public url without setting expire" do
      ...
      assert String.starts_with?(
               File.public_url("test-container", "test.png", connection: @connection),
               "http://test.test/test-container/test.png"
             )
    end
